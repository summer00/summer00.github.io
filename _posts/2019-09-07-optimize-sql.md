...
用完全随机生成的值作为标识符也会导致性能问题。INSERT 时由于值会插入到不同的位置，导致也分裂、磁盘随机访问，以及对于聚簇存储引起产生聚簇索引碎片。SELECT 语句也会变慢，因为可能逻辑上相近的数据分布在磁盘和内存不同的地方。还会导致缓存的效果变差，因为缓存的访问局部性原理失效（如果整个数据集都很“热”，则缓存部分没有好处；如果工作集比内存大，缓存将会有很多刷新和不命中）
