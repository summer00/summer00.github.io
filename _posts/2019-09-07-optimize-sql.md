...
基于 hash 表实现，对所有索引列计算 hash 值，hash 索引将这些值存储起来，并且保存该 hash 值指向的数据。当 hash 碰撞时，索引会以链表方式存放多个记录到同一个 hash 条目中。在 MySQL 中只有`Memory`引擎支持。
...
3. 分解关联查询
   ...
   4. 查询有可能会提升，如`in`替代关联查询，从随机读变为顺序读
   5. 减少冗余记录的查询
